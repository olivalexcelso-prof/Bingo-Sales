...
export const GameProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      // Fix: The following check ensures we only count down during non-playing states.
      if (gameState.status !== GameStatus.PLAYING && gameState.status !== GameStatus.FINISHED) {
        ...
        // Fix: Removed redundant comparison check on line 193 to fix type overlap error.
        if (secondsLeft <= 0) {
          forceStart();
        }
      }
